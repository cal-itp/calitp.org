...
This is an [11ty][11ty] static site. We include a [Dev Container][devcontainer] configuration to help with local testing
and development.
...
The site is now running on <http://127.0.0.1:8080>
...
The Resources index lists all items within the `resources` folder. 11ty generates a standard content page for each of these items, but if an item's front matter has an `asset` property, then the listing on the Resources index will link to whatever the value of that `asset` property is. Otherwise, it links to standard 11ty content.

Resource pages that are generated by 11ty for external assets are rendered with `<meta name="robots" content="noindex">` to prevent search engines from indexing them, and they redirect to the URL in their `asset` property.
...
The Press index lists all items within the `press` folder. 11ty generates a standard content page for each of these items, but if an item's front matter has an `external` property, then the listing on the Press index will link to whatever the value of that `external` property is. Otherwise, it links to standard 11ty content.

Press pages that are generated by 11ty for external articles are rendered with `<meta name="robots" content="noindex">` to prevent search engines from indexing them, and they redirect to the URL in their `external` property.
